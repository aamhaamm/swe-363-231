...
const fetchCitySuggestions = () => {
    ...
    fetch(`https://api.teleport.org/api/cities/?search=${query}`)
        ...
        .then(data => {
            ...
            const cities = data._embedded['city:search-results'].map(cityData => {
                return new City(cityData.matching_full_name.split(',')[0], cityData.matching_full_name);
            });
            cities.forEach(city => {
